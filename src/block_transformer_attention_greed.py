...
class AttODEblock_greed(ODEblock):
  def __init__(self, odefunc, regularization_fns, opt, data, device, t=torch.tensor([0, 1]), gamma=0.5):
    super(AttODEblock_greed, self).__init__(odefunc, regularization_fns, opt, data, device, t)
    ...
    self.multihead_att_layer = SpGraphTransAttentionLayer_greed(opt['hidden_dim'], opt['hidden_dim'], opt,
