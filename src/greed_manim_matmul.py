...
from manim.animation.creation import Create #ShowCreation
...
class NumericalMatrixMultiplication(Scene):
    def __init__(self):
        super(NumericalMatrixMultiplication, self).__init__()
        CONFIG = {
            "left_matrix": [[1, 2], [3, 4]],
            "right_matrix": [[5, 6], [7, 8]],
            "use_parens": True,
        }
        self.left_matrix = CONFIG['left_matrix']
        self.right_matrix = CONFIG['right_matrix']
        self.use_parens = CONFIG['use_parens']

    def construct(self):
        left_string_matrix, right_string_matrix = [
            np.array(matrix).astype("str")#ing")
            for matrix in (self.left_matrix, self.right_matrix)
            # for matrix in (CONFIG['left_matrix'], CONFIG['right_matrix'])
        ]
        if right_string_matrix.shape[0] != left_string_matrix.shape[1]:
            raise Exception("Incompatible shapes for matrix multiplication")
        print(left_string_matrix)
        print(right_string_matrix)
    ...
    def get_result_matrix(self, left, right):
        ...
        # mob_matrix = np.repeat(np.array([VGroup()]), m*n)#.reshape((m, n))
        print(mob_matrix)
        for a in range(m):
            for b in range(n):
                template = "(%s)(%s)" if self.use_parens else "%s%s"
                # template = "(%s)(%s)" if CONFIG['use_parens'] else "%s%s"
                parts = [
...
if __name__ == '__main__':
    #https://docs.manim.community/en/stable/contributing/performance.html#running-an-animation-as-a-script
    with tempconfig({"quality": "medium_quality"}): #https://stackoverflow.com/questions/70869433/debug-manim-community-in-pycharm
        scene = NumericalMatrixMultiplication()
