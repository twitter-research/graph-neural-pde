...
def set_photo_search_space(opt):
  ...
  opt["lr"] = tune.loguniform(0.005, 0.02)
  opt["input_dropout"] = tune.uniform(0.4, 0.8)
  opt["dropout"] = tune.uniform(0, 0.8)
  opt["time"] = tune.uniform(0.5, 5.0)
...
def set_arxiv_search_space(opt):
  ...
  opt["dropout"] = 0
  opt['step_size'] = tune.uniform(0.5, 2)
  opt['adjoint_step_size'] = tune.uniform(0.5, 2)
...
if __name__ == "__main__":
  ...
  parser.add_argument("--max_nfe", type=int, default=300, help="Maximum number of function evaluations in an epoch. Stiff ODEs will hang if not set.")
  # regularisation args
