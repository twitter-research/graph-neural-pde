...
def syn_cora_energy(path):
    df = pd.read_csv(path)
    gnl_df = df[(df.function == "greed_non_linear")]
    # gnl_piv = pd.pivot_table(gnl_df,
    #                          values=["T0_dirichlet_mean", "T0r_dirichlet_mean",
    #                                  "TN_dirichlet_mean", "TNr_dirichlet_mean"],
    #                          index="target_homoph", columns=["gnl_W_style"], aggfunc=np.max)
    gnl_df['W_homoph'] = df.apply(lambda row: str(row.gnl_W_style) + "_" + str(row.target_homoph), axis=1)
    gnl_piv = pd.pivot_table(gnl_df,
                             values=["T0_dirichlet_mean", "T0r_dirichlet_mean",
                                     "TN_dirichlet_mean", "TNr_dirichlet_mean"],
                             index="W_homoph", aggfunc=np.max)

    fig, ax = plt.subplots()
    tips = sns.load_dataset("tips")
    sns.catplot(x="day", y="total_bill", data=tips)
    # sns.catplot(x="W_homoph", y="value", jitter=False, data=gnl_piv)
    fig.show()
