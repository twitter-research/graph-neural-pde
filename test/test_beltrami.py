...
class BeltramiTests(unittest.TestCase):
  ...
  def test_pos_enc(self):
    pos_encoding = apply_gdc(self.dataset.data, self.opt, type='position_encoding')
    self.assertTrue(pos_encoding.shape == (self.dataset.data.num_nodes, self.dataset.data.num_nodes))
    self.assertTrue(torch.all(torch.isclose(pos_encoding.sum(dim=0), torch.ones((self.dataset.data.num_nodes, self.dataset.data.num_nodes)))))
