...
from function_transformer_attention_greed import SpGraphTransAttentionLayer_greed, ODEFuncTransformerAttGreed
...
class AttentionTests(unittest.TestCase):
  ...
  def test_symmetric_attention_QK(self):
  ...
  def test_symmetric_attention(self):
    self.opt['symmetric_attention'] = True
    in_features = self.x1.shape[1]
    out_features = self.x1.shape[1]
    att_layer = SpGraphTransAttentionLayer_greed(in_features, out_features, self.opt, self.device, concat=True)
    attention, _ = att_layer(self.x1, self.edge1)  # should be n_edges x n_heads
    mean_attention = attention.mean(dim=1)
    dense_att = to_dense_adj(self.edge1, edge_attr=mean_attention)
    dense_att_T = dense_att

    self.assertTrue(torch.all(torch.eq(dense_att,dense_att_T)))
