...
import numpy as np
from utils import sparse_dense_mul, get_rw_adj, get_rw_adj_old, gcn_norm_fill_val
...
class UtilsTests(unittest.TestCase):
  def setUp(self):
    ...
    self.opt = {'dataset': 'Cora', 'self_loop_weight': 0, 'leaky_relu_slope': 0.2, 'beta_dim': 'vc', 'heads': 2,
                'K': 10,
  ...
  def test_get_rw_adj(self):
    ...
    # edge_index, edge_weight = get_rw_adj(self.edge, norm_dim=1, fill_value=self.opt['self_loop_weight'],
    #                                      num_nodes=self.x.shape[0])
    # dense_rw_adj = to_dense_adj(edge_index, edge_attr=edge_weight)
    # print('dense rw adj', dense_rw_adj)
    # rw_adj_old_scipy = get_rw_adj_old(self.data, self.opt).to_dense().numpy().squeeze()
    # print('dense rw adj scipy', rw_adj_old_scipy)
    # self.assertTrue(np.allclose(rw_adj_old_scipy, dense_rw_adj.numpy().squeeze()))
    #
    ...
    # print('dense rw adj scipy', rw_adj_old_scipy)
    # self.assertTrue(np.allclose(rw_adj_old_scipy, dense_rw_adj.numpy().squeeze()))
    # self.opt['self_loop_weight'] = 2.1
    # edge_index, edge_weight = get_rw_adj(self.edge, norm_dim=1, fill_value=self.opt['self_loop_weight'],
    #                                      num_nodes=self.x.shape[0])
    # dense_rw_adj = to_dense_adj(edge_index, edge_attr=edge_weight)
    # rw_adj_old_scipy = get_rw_adj_old(self.data, self.opt).to_dense().numpy().squeeze()
    # self.assertTrue(np.allclose(rw_adj_old_scipy, dense_rw_adj.numpy().squeeze()))
    # self.opt['self_loop_weight'] = -3.2
    # edge_index, edge_weight = get_rw_adj(self.edge, norm_dim=1, fill_value=self.opt['self_loop_weight'],
    #                                      num_nodes=self.x.shape[0])
    # dense_rw_adj = to_dense_adj(edge_index, edge_attr=edge_weight)
    # rw_adj_old_scipy = get_rw_adj_old(self.data, self.opt).to_dense().numpy().squeeze()
